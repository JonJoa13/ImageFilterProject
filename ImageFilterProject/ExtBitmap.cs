...
namespace ImageFilterProject
{
    public static class ExtBitmap
    {
        ...
        private static Bitmap ConvolutionFilter(Bitmap sourceBitmap, double[,] filterMatrix, double factor = 1, int bias = 0, bool grayscale = false)
        {
            ...
            if (grayscale == true)
            {
                RgbMethod(pixelBuffer);
            }

            OffsetsWithFilterBGR_X(sourceBitmap, filterMatrix, sourceData, pixelBuffer, resultBuffer, factor, bias);
        ...
        public static Bitmap ConvolutionFilter(this Bitmap sourceBitmap, double[,] xFilterMatrix, double[,] yFilterMatrix, double factor = 1, int bias = 0, bool grayscale = false)
        {
            ...
            OffsetsWithFilterBGR_XY(sourceBitmap, xFilterMatrix, yFilterMatrix, sourceData, pixelBuffer, resultBuffer, factor, bias);
        ...
        private static void RgbMethod(byte[] pixelBuffer)
        {
            float rgb = 0;
        ...
        private static void OffsetsWithFilterBGR_X(Bitmap sourceBitmap, double[,] filterMatrix, BitmapData sourceData, byte[] pixelBuffer, byte[] resultBuffer, double factor = 1, int bias = 0)
        {
            double blue = 0.0;
            double green = 0.0;
            double red = 0.0;
        ...
        private static void OffsetsWithFilterBGR_XY(Bitmap sourceBitmap, double[,] xFilterMatrix, double[,] yFilterMatrix, BitmapData sourceData, byte[] pixelBuffer, byte[] resultBuffer, double factor = 1, int bias = 0)
        {
            double blueX = 0.0;
            double greenX = 0.0;
            double redX = 0.0;
        ...
        public static Bitmap BlackWhiteFilter(this Bitmap initialBmp)
        {
            Bitmap Bmp = new Bitmap(initialBmp);
            int rgb;
            Color c;
        ...
        //Each pixel of the two images will be compare
        //If each pixel are identical, it will return true
        public static bool CompareTwoImages(Bitmap img1, Bitmap img2)
        {
            ...
            string secondPixel;
            bool flag = true;
            //check if the two image are the same size. If not, no need to compare the pixels
...

            {
                for (int y = 0; y < img1.Height; y++)
                {
                    ...
                    {
                        firstPixel = img1.GetPixel(x, y).ToString();
                        secondPixel = img2.GetPixel(x, y).ToString();
